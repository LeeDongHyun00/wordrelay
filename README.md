...
- **정답:** 통합 사전 802,038개. 국어 명사·구·롤 챔피언·클래시 로얄 카드·애니 제목을 모드 구분 없이 인정합니다. 게임 화면에서 뜻, 인정 근거, 출처를 펼쳐 볼 수 있습니다.
...
2. 3초 카운트다운 후 무작위 선공과 시작 단어를 제시합니다. 시작 단어는 고정 목록이 아닌 사전의 2~5글자 단어 중 다른 단어로 이어갈 수 있는 후보에서 무작위로 선택합니다. 퇴장하지 않은 참가자의 차례가 돌아옵니다.
3. 서버에 도착한 정답을 기준으로 `max(1000, 6000 - 정답수 × 100)` 밀리초를 다음 차례에 적용합니다. 표시 시간은 첫 차례 6초, 첫 정답 후 5.9초, 50번째 정답 후부터 1초입니다. 2→1초는 ½속도, 1→0초는 ¼속도로 흐릅니다. 실제 제한 시간은 표시 6초일 때 10초, 표시 1초일 때 4초입니다. 서버 마감 시각도 같은 감속을 적용합니다.
4. 오답은 시간을 줄이거나 초기화하지 않습니다. 제한 시간 안에 재입력할 수 있습니다. 이전 차례의 지연·중복 제출은 `turnId`로 거절합니다. 마감 시각과 같은 시각에 처리되는 답은 시간 초과입니다.
5. 한 명이 시간 초과하거나 게임 중 퇴장하면 즉시 라운드가 종료됩니다. 나머지 참가자는 각각 300점 보너스를 받습니다. 4초 결과 안내 후 모든 잔류 참가자가 다음 라운드에 다시 참가합니다. 마지막 라운드 뒤 누적 점수로 우승자를 정합니다.
6. 한방 단어도 정답입니다. 후속 단어가 없어도 입력한 단어를 그대로 유지하고 다음 참가자에게 차례를 넘깁니다. 제한시간이 지나면 해당 참가자의 실패로 라운드가 끝납니다.
7. 한 글자, 공백·문장부호를 제거한 표기, 등록된 한글 읽기를 인정합니다. 두음법칙은 정방향만 적용합니다. 같은 정답은 한 게임의 모든 라운드에서 다시 사용할 수 없습니다.
...
10. 게임 중 직접 나가면 해당 라운드가 즉시 끝납니다. 종료 후 방장이 ‘다시 준비하기’를 누르면 연결된 참가자와 같은 방에서 다시 준비합니다.
...
- 방장은 대기방과 게임 화면에서 다른 참가자의 ‘강퇴’를 눌러 내보낼 수 있습니다. 게임 중에는 해당 라운드를 끝내고 나머지 참가자와 다음 라운드로 진행합니다. 기존 재접속 토큰은 무효입니다. 로그인 없는 서비스여서 새 참가자로 다시 입장하는 것까지 차단하는 영구 이용 정지는 아닙니다.
...
방장이 대기방에서 1~10라운드(기본 3)를 설정합니다. 설정 변경 시 준비가 해제됩니다. 각 라운드는 3초 카운트다운 뒤 6초로 시작하며, 정답마다 0.1초 감소해 최소 1초가 됩니다. 참가 상태는 매 라운드 초기화하지만, 시작 단어와 정답의 사용 기록은 게임 전체에서 유지합니다. 이미 나온 단어는 이후 라운드에서도 사용할 수 없으며, 새 게임을 시작하면 기록을 초기화합니다. 표시는 2초 아래에서 ½속도, 1초 아래에서 ¼속도로 줄어들고 실제 서버 제한 시간도 연장됩니다.
...
실패 감점 = min(150, floor(이번 라운드 획득 점수 × 0.2)). 이전 라운드 점수를 보존하고, 0점 미만으로 내려가지 않으며 라운드 종료 후 퇴장해도 중복 차감하지 않습니다. 시간 초과·경기 중 퇴장·강퇴에 같은 규칙을 적용합니다. 실패한 사람을 제외한 잔류 참가자 각각에게 300점 보너스를 줍니다.
...
`data/dictionary.json`의 `supplements`에 등록된 `opendict.json.gz`를 서버가 시작할 때 함께 읽습니다. 압축 자료는 전체 XML에서 추출한 추가 명사·구 표기와 대표 뜻풀이이며, 기존 게임·역·작품 데이터는 유지합니다. 원본 스냅샷과 필터별 집계는 `data/opendict-import-report.json`에 있습니다. 사전 전체 항목 수와 게임에서 인정하는 고유 표기 수는 다릅니다.
...
모션: 정답 글자·점수·다음 글자, 오답 입력창, 화면 진입, 준비·차례 전환에 상태 기반 애니메이션을 적용합니다. 연결 신호마다 재생하지 않으며 `prefers-reduced-motion`에서는 움직임을 끕니다. 타이머는 정상/감속/마지막 구간을 색상·문구·소수점 정밀도로 구분합니다.

모션은 입력을 막지 않는 짧은 피드백으로 구성합니다. 정답 글자는 270ms 이내에 정착하며 득점 표시는 해당 참가자 카드에 나타납니다. 오답은 입력창만 짧게 흔들리고 경고는 입력창 아래에 표시됩니다. 화면 전체 플래시는 사용하지 않으며, 화면 갱신 시 이전 모션을 취소하고 기기의 동작 줄이기 설정을 실시간 반영합니다.

확장 사전은 명사와 사전 단위가 `구`인 표제어를 함께 수집합니다. 현재 802,038개 정답을 포함하며, 사전 조회용 키와 연결 정보는 메모리에, 뜻풀이는 서버 시작 시 생성한 프로세스 전용 임시 파일에 보관합니다. 정답 제출 시 필요한 뜻만 읽으므로 전체 뜻풀이를 메모리에 유지하지 않습니다. 임시 파일은 이름을 즉시 제거하고 열린 핸들로만 접근하며 서버 종료 시 회수됩니다.
