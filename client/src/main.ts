...
import {enterView,turnMotion,feedbackMotion,changedPlayer,disclosure,clearMotion} from './motion';
...
let pendingMotion:'success'|'error'|null=null, motionPoints=0, motionPlayerId:string|null=null;
...
function connect(request:object){
  ...
  socket.onmessage=({data})=>{
    ...
    if(message.type==='welcome'){
      ...
      if(previous?.code===room!.code&&previous.round===room!.round&&room!.acceptedCount>previous.acceptedCount){pendingMotion='success';motionPoints=room!.history.at(-1)?.points||0;motionPlayerId=room!.history.at(-1)?.playerId||null;}
...
function render(){
  ...
  if(oldView!==view)enterView(app);
    ...
    if(oldTurn!==app.dataset.turn)turnMotion(app,pendingMotion==='success');
  ...
  if(pendingMotion&&room?.phase==='playing'){feedbackMotion(app,pendingMotion,motionPoints,motionPlayerId);pendingMotion=null;}
}
