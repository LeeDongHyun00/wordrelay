...
function lobby(){const r=room!;const me=r.players.find(p=>p.id===session?.playerId);const host=r.hostId===session?.playerId;
 return `<main class="room-page"><div class="room-toolbar"><button class="text-button" data-action="leave">← 나가기</button><span class="room-code">ROOM <b>${r.code}</b><button aria-label="초대 코드 복사" data-action="copy-code">⧉</button></span></div><div class="lobby-layout"><section class="lobby-main"><h1>대기방</h1><p class="muted">모두 준비하면 방장이 시작할 수 있습니다.</p><div class="section-title"><h3>플레이어 <span>${r.players.length} / 4</span></h3><span>입장 후 준비를 눌러 주세요</span></div><div class="players lobby-players">${Array.from({length:4},(_,i)=>playerCard(r.players[i],i)).join('')}</div><div class="round-settings"><label for="round-count">라운드 수</label><select id="round-count" ${!host||!connected?'disabled':''}>${Array.from({length:10},(_,i)=>`<option value="${i+1}" ${r.totalRounds===i+1?'selected':''}>${i+1} 라운드</option>`).join('')}</select><button class="outline" data-action="sound">알림 소리 ${soundEnabled?'켜짐':'꺼짐'}</button></div><details class="scoring-rules"><summary>점수 규칙</summary><p>정답 100점 + 길이 20점 × (글자 수 − 1) + 속도 최대 200점</p><p>속도 점수는 남은 시간 비율에 비례합니다. 글자 수는 등록된 한글 읽기 기준이며 공백·기호는 제외합니다. 라운드 승리 +300점. 탈락하면 이번 라운드 점수의 20% 차감(소수점 버림, 최대 150점). 이전 라운드 점수는 보호됩니다. 누적 점수 동점은 공동 우승입니다.</p><p>예: 4글자 단어를 제한시간의 절반 안에 입력하면 260점.</p></details><div class="lobby-actions"><button class="${me?.ready?'ready-button':'dark'}" data-action="ready" ${!connected?'disabled':''}>${me?.ready?'✓ 준비 완료 · 취소하기':'준비하기'}</button>${host?`<button class="primary" data-action="start" ${!r.canStart||!connected?'disabled':''}>게임 시작 <span>→</span></button>`:`<span class="waiting-label">방장이 시작하기를 기다리고 있어요.</span>`}</div><p class="small-note">라운드마다 6초부터 시작 · 정답마다 0.1초 감소 · 최소 1초</p></section><aside class="invite-card"><h2>QR로 입장</h2><p>카메라로 스캔하면 이 방에 입장합니다.</p><div class="qr-frame"><img id="qr" width="200" height="200" alt="이 대기방에 입장하는 QR 코드" ${qrCache?`src="${qrCache}"`:''}><span class="qr-center-label">SCAN & JOIN</span></div><div class="invite-code">${r.code}</div><button class="outline" data-action="copy-link">초대 링크 복사 <span>⧉</span></button><input class="invite-url" readonly aria-label="초대 주소" value="${escape(inviteUrl())}">${['localhost','127.0.0.1'].includes(location.hostname)?'<p class="local-note">휴대폰 초대는 같은 Wi-Fi의 컴퓨터 IP 주소나 배포 주소로 접속해 주세요.</p>':''}</aside></div></main>`;
}
...
function finished(){const r=room!,between=r.phase==='intermission',winner=r.players.find(p=>p.id===r.winnerId);const names=r.winnerIds.map(id=>r.players.find(p=>p.id===id)?.name||'').map(escape).join(' · ');
return `<main class="results-page"><div class="room-toolbar"><button class="text-button" data-action="leave">← 나가기</button><span class="room-code">ROOM <b>${r.code}</b></span></div><section class="result-card"><span class="eyebrow">${between?`${r.round} / ${r.totalRounds} 라운드 결과`:'최종 결과'}</span><h1>${between?(winner?`${escape(winner.name)} 라운드 승리`:'라운드 종료'):(names?`${names}<br>${r.winnerIds.length>1?'공동 우승':'최종 우승'}`:'게임 종료')}</h1>${between?`<p class="round-countdown" role="status">다음 라운드까지 <b id="round-wait">4</b>초</p><p>라운드 승리 보너스 +300점</p>`:`<p>${r.round} / ${r.totalRounds} 라운드 진행${r.round<r.totalRounds?' · 참가자 부족으로 조기 종료':''}</p>`}<h3 class="score-heading">누적 점수</h3>${scoreTable()}<details class="round-breakdown"><summary>라운드별 점수</summary>${r.roundResults.map(result=>`<div><h3>${result.round} 라운드</h3>${result.scores.map(s=>`<p>${escape(r.players.find(p=>p.id===s.playerId)?.name||'퇴장한 참가자')} <b>+${s.points}점</b>${s.penalty?` · 탈락 감점 −${s.penalty}점 반영`:''}${result.winnerId===s.playerId?' · 라운드 승리':''}</p>`).join('')}</div>`).join('')}</details>${!between&&r.hostId===session?.playerId?'<button class="primary large" data-action="rematch">다시 준비하기 <span>↗</span></button>':''}</section></main>`;
}
function render(){
